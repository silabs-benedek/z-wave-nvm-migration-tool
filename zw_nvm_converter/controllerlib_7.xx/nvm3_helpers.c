...
bool nvm3_check_files(nvm3_Handle_t *nvm3_handle, const nvm3_file_descriptor_t *filelist, uint32_t filelist_size)
{
  ...
  bool at_least_a_file_ok = false;

  for (uint32_t i = 0; i < filelist_size; i++)
  {
    fdesc = (nvm3_file_descriptor_t*)&filelist[i];
    if (fdesc->optional)
    {
    ...
    if (ECODE_NVM3_OK != ec)
    {
      user_message(MSG_ERROR, "WARNING nvm3 file %s (0x%x) not found\n", fdesc->name, fdesc->key);
      all_files_ok_status = false;
    }
    else if (NVM3_OBJECTTYPE_DATA != type)
    {
      user_message(MSG_ERROR, "WARNING nvm3 file %s (0x%x) not of type DATA\n", fdesc->name, fdesc->key);
      all_files_ok_status = false;
    }
    else if (fdesc->size != size)
    {
      if (fdesc->key == FILE_ID_APPLICATIONCONFIGURATION &&
         (size == FILE_SIZE_APPLICATIONCONFIGURATION_7_18_1 ||
          size == FILE_SIZE_APPLICATIONCONFIGURATION_7_21_x)) {
        // Update the actual size
        fdesc->size = size;
      } else {
        user_message(MSG_ERROR, "WARNING nvm3 file %s (0x%x) size is %d. The expected size is %d\n", fdesc->name, fdesc->key, size, fdesc->size);
        all_files_ok_status = false;
      }
    } else {
      at_least_a_file_ok = true;
    }
  }
  if (!all_files_ok_status && at_least_a_file_ok)
    return true;
...
bool key_is_SUC_NODE_LIST_V5(nvm3_ObjectKey_t key)
{
  if (FILE_ID_SUCNODELIST_BASE_V5 > key)
  {
    return false;
  }
  if ((FILE_ID_SUCNODELIST_BASE_V5 + SUC_MAX_UPDATES/SUCNODES_PER_FILE- 1) < key)
  {
    return false;
  }
  return true;
